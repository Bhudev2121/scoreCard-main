document.addEventListener("DOMContentLoaded", function () 
{
  ...
  function updateCurrentOverDisplay() 
  {
    ...
    matchData.currentOver.forEach((ball) => {
      ...
      {
        ballElement.classList.add("bg-blue-100", "text-blue-700");
      ...
      else 
      {
        if(ball.runs == 6)
        {
          ballElement.classList.add("bg-green-100", "text-green-700");
        }
        if(ball.runs == 4)
        {
          ballElement.classList.add("bg-yellow-100", "text-yellow-700");
        }
